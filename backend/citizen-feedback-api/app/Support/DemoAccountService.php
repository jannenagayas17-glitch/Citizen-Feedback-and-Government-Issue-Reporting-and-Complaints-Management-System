...
class DemoAccountService
{
    ...
    public function previewDemoUsers(): Collection
    {
        return $this->scopeDemoUsers(
            User::withTrashed()
                ->orderByRaw(
                    "case when role = 'super_admin' then 0 when role = 'admin' then 1 when role = 'administrative_staff' then 2 when role = 'front_desk' then 3 when role = 'pending_admin' then 4 else 5 end"
                )
                ->orderBy('email')
        )->get();
    }
    ...
    /**
     * @return array{citizen:int,admin:int,administrative_staff:int,pending_admin:int,super_admin:int,other:int}
     */
    ...
    private function roleBucket(string $role): string
    {
        return match (User::normalizeRole($role)) {
            'citizen', 'admin', User::ROLE_ADMINISTRATIVE_STAFF, 'pending_admin', 'super_admin' => User::normalizeRole($role),
            default => 'other',
        };
    }
}
